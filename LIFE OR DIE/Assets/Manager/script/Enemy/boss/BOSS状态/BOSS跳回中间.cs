...
public class BOSS跳回中间 :BOSS状态基类
{
    public BOSS跳回中间(BOSSAI控制器 aIFsm) : base(aIFsm)
    {
    }
    private const float 上升距离 = 8f;
    private float 目标位置X=20.4f;
    private float 目标位置Y;
    ...
    private void 上升()
    {
        ...
        float X = Mathf.Lerp(AIFsm.transform.position.x, 目标位置X, scale);
