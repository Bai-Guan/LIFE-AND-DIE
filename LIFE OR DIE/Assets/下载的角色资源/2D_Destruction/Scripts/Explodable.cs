...
public class Explodable : MonoBehaviour
{
    ...
    public void explode(DamageData damage,Vector2 Dir2)
    {
        //if fragments were not created before runtime then create them now
        if (fragments.Count == 0 && allowRuntimeFragmentation)
        {
        ...
        else
        {
            foreach (GameObject frag in fragments)
            {
                ...
                ApplyFragmentVelocity(frag,damage,Dir2);
            }
        }
        //if fragments exist destroy the original
        if (fragments.Count > 0)
    ...
    private void ApplyFragmentVelocity(GameObject fragment, DamageData damageData,Vector2 dir)
    {
        const float SpeedMulX = 3f;
        const float SpeedMulY = 1.5f;
